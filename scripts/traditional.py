...
def supported_letters(last_char):
    '''This function changes the valid "last character" to a list of valid
     "last characters" that the user or program can start their poems with it'''

    if last_char in ['س', 'ص', 'ث']:
        last_char = ['س', 'ص', 'ث']
    elif last_char in ['ذ', 'ز', 'ض', 'ظ']:
        last_char = ['ذ', 'ز', 'ض', 'ظ']
    elif last_char in ['ت', 'ط']:
        last_char = ['ت', 'ط']
...
def start(self, app, stt, speaker):
    ...
    while 1:
        ...
        for i in poems:
            if similar(user_poem.replace(' / ', ' '), i) > 0.4:
                user_poem = i
                ...
                user_first_char = supported_letters(user_poem[0])
                user_last_char = supported_letters(user_poem[-1])
        ...
        current_last_char = supported_letters(machine_poem[-1][-1])
        ...
        self.speak.setText('،'.join(current_last_char))
        app.processEvents()